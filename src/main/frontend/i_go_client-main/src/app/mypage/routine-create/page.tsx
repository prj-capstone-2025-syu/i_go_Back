  ...
  useEffect(() => {
    const loadRoutines = async () => {
      try {
        ...
        console.log('루틴 데이터:', routineData); // 디버그용

        // 데이터가 배열이고 각 항목이 올바른 구조인지 확인
        if (Array.isArray(routineData)) {
          const validRoutines = routineData.filter(routine =>
              routine &&
                    ...
                    <div className="relative">
                      ...
                      <div className="group relative flex w-full justify-between items-center overflow-hidden">
                        <select
                            ...
                            className="appearance-none bg-transparent w-full h-full outline-none border-[1px] border-[#DFDFDF] focus:border-[#383838] pl-[15px] pr-[42px] py-[8px] text-[13px] rounded-[4px] disabled:bg-gray-100"
                        >
                          <option value="">
                            {routinesLoading ? '루틴 로딩 중...' : '루틴을 선택하세요 (선택사항)'}
                          </option>
                          {!routinesLoading && routines.length > 0 && routines.map((routine) => (
                              <option key={routine.id} value={routine.id}>
                                {routine.name}
                              </option>
                          ))}
                          {!routinesLoading && routines.length === 0 && (
                              <option value="" disabled>등록된 루틴이 없습니다</option>
                          )}
                        </select>
